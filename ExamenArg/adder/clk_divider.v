module clk_divider #(parameter freq = 50000)(
    input clk, rst,
    output reg clk_div);

    reg [31:0] count; // Contador para dividir el reloj

    parameter clk_freq = 50000000; // Frecuencia del reloj de entrada en Hz (ajustar según tu caso)
    ...
    always @(posedge clk or posedge rst) 
        begin
            if (rst == 1)
                count  = 32'b0; // Reiniciar el contador
    ...
    always @(posedge clk or posedge rst)
        begin
            if (rst == 1) //la señal de clock div la mandamos a 0 con una no bloqueante para que se reinicie inmediatamente al activar el reset
